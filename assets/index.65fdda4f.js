...
`;function Ql(t,e){return new PIXI.Filter(void 0,Vl,{width:t,height:e})}const D=new m;let S,L,w,R=new m;const el=5;document.createElement("img");let b,me,pe=1,ht="black",M,tl,se=!1,p,ll;function Zl(t,e,l,i,s,r,o){D.set(t),p=o,ht=l;const a=`
...
`;if(document.body.style.cssText=a,R.set(D),p.isUsingPixi){R.mul(el);const y=new PIXI.Application({width:R.x,height:R.y});if(S=y.view,w=new PIXI.Graphics,w.scale.x=w.scale.y=el,PIXI.settings.SCALE_MODE=PIXI.SCALE_MODES.NEAREST,y.stage.addChild(w),w.filters=[],p.name==="crt"&&w.filters.push(ll=new PIXI.filters.CRTFilter({vignettingAlpha:.7})),p.name==="pixel"&&w.filters.push(Ql(R.x,R.y)),p.name==="pixel"||p.name==="shapeDark"){const k=new PIXI.filters.AdvancedBloomFilter({threshold:.1,bloomScale:p.name==="pixel"?1.5:1,brightness:p.name==="pixel"?1.5:1,blur:8});w.filters.push(k)}w.lineStyle(0),S.style.cssText=d}else S=document.createElement("canvas"),S.width=R.x,S.height=R.y,L=S.getContext("2d"),L.imageSmoothingEnabled=!1,S.style.cssText=d+u;document.body.appendChild(S);const g=()=>{const k=innerWidth/innerHeight,j=R.x/R.y,W=k<j,E=W?.95*innerWidth:.95*innerHeight*j,ot=W?.95*innerWidth/j:.95*innerHeight;S.style.width=`${E}px`,S.style.height=`${ot}px`};if(window.addEventListener("resize",g),g(),i){b=document.createElement("canvas");let y;s?(b.width=R.x,b.height=R.y,y=r):(R.x<=R.y*2?(b.width=R.y*2,b.height=R.y):(b.width=R.x,b.height=R.x/2),b.width>400&&(pe=400/b.width,b.width=400,b.height*=pe),y=Math.round(400/b.width)),me=b.getContext("2d"),me.fillStyle=e,gcc.setOptions({scale:y,capturingFps:60,isSmoothingEnabled:!1})}}function Ue(){if(p.isUsingPixi){w.clear(),w.beginFill(ne(ht,p.isDarkColor?.15:1)),w.drawRect(0,0,D.x,D.y),w.endFill(),w.beginFill(ne(M)),se=!0;return}L.fillStyle=ge(ht,p.isDarkColor?.15:1),L.fillRect(0,0,D.x,D.y),L.fillStyle=ge(M)}function U(t){if(t===M){p.isUsingPixi&&!se&&Ke(ne(M));return}if(M=t,p.isUsingPixi){se&&w.endFill(),Ke(ne(M));return}L.fillStyle=ge(t)}function Ke(t){Ye(),w.beginFill(t),se=!0}function Ye(){se&&(w.endFill(),se=!1)}function Ae(){tl=M}function Je(){U(tl)}function ye(t,e,l,i){if(p.isUsingPixi){p.name==="shape"||p.name==="shapeDark"?w.drawRoundedRect(t,e,l,i,2):w.drawRect(t,e,l,i);return}L.fillRect(t,e,l,i)}function ei(t,e,l,i,s){const r=ne(M);Ke(r),w.drawCircle(t,e,s*.5),w.drawCircle(l,i,s*.5),Ye(),w.lineStyle(s,r),w.moveTo(t,e),w.lineTo(l,i),w.lineStyle(0)}function ti(){ll.time+=.2}function li(){if(me.fillRect(0,0,b.width,b.height),pe===1)me.drawImage(S,(b.width-S.width)/2,(b.height-S.height)/2);else{const t=S.width*pe,e=S.height*pe;me.drawImage(S,(b.width-t)/2,(b.height-e)/2,t,e)}gcc.capture(b)}const il=[`
l
l
l
